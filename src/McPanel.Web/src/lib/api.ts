...
export const api = {
  ...
  recovery: () => request<{ configured: boolean; intervalMinutes: number; points: Array<{ id: string; createdAt: string; size: number; verifiedAt?: string; replicatedAt?: string; error?: string; includesInstances?: boolean }> }>("/recovery"),
  ...
  exportInstances: (selection: { all: boolean; serverIds?: string[] }) => request<JobDto>("/exports/instances", { method: "POST", body: JSON.stringify(selection) }),
