...
const actionTypes: ScheduleActionType[] = ["Start", "Stop", "Restart", "Backup", "Update", "Command"]
          ...
          <FieldGroup>
            {form.actions.map((action, index) => (
              <Card key={`${index}-${action.action}`} size="sm">
                ...
                <CardContent className="flex flex-col gap-3 sm:flex-row"><Select items={actionTypes.map((value) => ({ value, label: value }))} value={action.action} onValueChange={(value) => value && updateAction(index, { action: value as ScheduleActionType, command: value === "Command" ? action.command : undefined })}><SelectTrigger className="sm:w-44" aria-label={`Action for step ${index + 1}`}><SelectValue /></SelectTrigger><SelectContent><SelectGroup>{actionTypes.map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectGroup></SelectContent></Select>{action.action === "Command" && <Input aria-label={`Command for step ${index + 1}`} maxLength={4096} value={action.command ?? ""} placeholder="say Backup complete" onChange={(event) => updateAction(index, { command: event.target.value.replace(/[\r\n]/g, "") })} />}</CardContent>
              </Card>
            ))}
...
export function SchedulesPage() {
  ...
  const schedules = useQuery({ queryKey: ["schedules", serverId], queryFn: () => api.schedules(serverId) })
