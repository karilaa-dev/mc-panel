...
{
    public async Task<IReadOnlyList<ModFileDto>> ListAsync(Guid serverId, CancellationToken cancellationToken)
    {
        ...
        if (server.Kind is not (ServerKind.Fabric or ServerKind.Forge or ServerKind.NeoForge))
            throw PanelProblems.Validation("Mods are available only for Fabric, Forge, and NeoForge servers.");

        var directory = Path.Combine(paths.Instance(serverId), "mods");
        ...
        foreach (var file in files)
        {
            ...
            result.Add(await ReadFileAsync(server.Kind, file, cancellationToken));
        }
        return result;
    }
    ...
    private static ModFileDto Unrecognized(FileInfo file) =>
        new(file.Name, SafeLength(file), null, ModParseStatus.Unrecognized, "No supported loader metadata was found.", null, []);
