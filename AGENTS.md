...
- When deployment is required, deploy the current checkout to the system installation with `./mcpanel.sh`. Use `install` when MC Panel is not installed and `update` when an installation already exists.
